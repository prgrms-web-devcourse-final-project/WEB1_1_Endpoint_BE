...
class UserServiceTest :
        DescribeSpec({
            ...
            // System Under Test
            val userService =
                    UserService(userAppender, userReader, userRemover)
            ...
            describe("getUser 에서") {
                context("존재하는 ID의 유저가 주어졌을 때") {
                    // Context
                    every { userReader.read(userId.value) } returns
                            user
                ...
                context("존재하지 않는 ID의 유저가 주어졌을 때") {
                    ...
                    every { userReader.read(nonExistentId) } throws
                            RuntimeException()
